...
- `AMEND`: the task explicitly targets one named existing proposed authority and revises it without expanding or changing that proposal's already-declared authority scope, authority ownership, or bounded Decision identity; or an accepted authority receives a strictly additive obligation under unchanged Goal, scope, authority ownership, and accepted Decisions, using new stable IDs while preserving every existing Decision and Contract meaning.
...
- `NEW`: no active accepted Product Authority owns the bounded independent long-lived decision, and either the task does not target a named existing proposed authority or the requested change would expand or change that proposal's already-declared authority scope, authority ownership, or bounded Decision identity. `NEW` creates a separate authority for the new or expanded decision.

Proposal-target identity takes precedence over the absence of accepted authority only while the proposal's already-declared authority scope, authority ownership, and bounded Decision identity remain unchanged. A named proposal MUST NOT force `AMEND` when the requested change expands or changes any of those boundaries; that complete input is `NEW`. Changing existing accepted Contract meaning or failure semantics is `SUPERSEDE`, not `AMEND` or `NEW`. The mere existence of an unrelated or abandoned proposal does not trigger `AMEND`; the task must name that proposal as its exact authoring target.

`AMEND_OR_NEW_PENDING_OWNERSHIP` MAY be used only while authority ownership, proposal-target identity, and proposed-boundary impact are being investigated. Before `AUTHORING_READY_FOR_REVIEW = YES`, `IMPLEMENTATION_ALLOWED = YES`, `MERGE_READY = YES`, or `OPERATION_ALLOWED = YES`, it MUST resolve to exactly one Authority action.
...
- Method: route a complex internal refactor under an implementation-authorizing Contract, a simple controlled one-shot operation, a revision whose exact target is a named existing proposed authority within its already-declared scope/ownership/Decision identity, the same named proposal with expanded authority scope, the same named proposal with changed authority ownership, the same named proposal with unchanged scope and ownership but changed bounded Decision identity, the same independent decision when no proposed authority is the task target, a strictly additive same-scope obligation on accepted authority, an independent new permission obligation, a change to existing accepted meaning, and an accepted Program with `implementation_authority: none`
- Environment: exact candidate Head
- Required evidence: exact accepted/proposed authority inventory and revisions; the named proposal's already-declared scope, ownership, and bounded Decision identity; the requested scope, ownership, and Decision identity; the declared authoring target or explicit absence of one; one classification matrix showing a unique Authority action, Plan level, Assurance level, primary authority, proposal-target identity, proposal-boundary impact, and implementation-authority result for every case; the changed-Decision-identity case's original and requested Decision identities, `NEW` classification, and negative rejection of `AMEND`; plus a negative-control result for each incorrect route
- Expected result: complexity changes Plan only; risk changes Assurance only; in-bound revision of the named existing proposal is `AMEND`; named-proposal scope expansion is `NEW`; named-proposal ownership change is `NEW`; named-proposal bounded Decision-identity change is `NEW`; the same decision without an existing proposed target is `NEW`; the additive accepted-authority case is `AMEND`; the independent obligation is `NEW`; changed existing accepted meaning is `SUPERSEDE`; `REUSE` changes no Contract meaning; the one-shot remains Brief plus Controlled Runbook; and the accepted non-authorizing Program does not permit child implementation
- Failure condition: any one axis automatically forces another; a named proposal with unchanged declared boundaries yields `NEW`; a named proposal with expanded scope, changed ownership, or changed bounded Decision identity yields `AMEND`; a complete input without a named proposed target yields `AMEND` merely because another proposal exists; the same complete input legally yields both `AMEND` and `NEW`; Investigation detail becomes a Contract; risk alone creates an ExecPlan/platform; partial supersession is inferred from prose; or `implementation_authority: none` authorizes implementation
